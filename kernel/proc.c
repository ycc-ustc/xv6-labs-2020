...
userinit(void)
{
  ...
  k_copy_mapping(p->pagetable, p->kernel_pgtbl, 0, p->sz);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    if(k_copy_mapping(p->pagetable, p->kernel_pgtbl, sz, n) != 0) {
...
fork(void)
{
  ...
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0 ||
    k_copy_mapping(np->pagetable, np->kernel_pgtbl, 0, p->sz) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
